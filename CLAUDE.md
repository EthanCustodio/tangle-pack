...
- **Fill.** On stable branch `j`, let `T_j` be its outermost crossing and `T_{j-1}` the outermost crossing of the branch one step back along `advance_key`. The arcs filled (passable to a walk) are those with cdist above `cdist(f(T_{j-1}))` up to `T_j`. With every branch trimmed at the strong-pip orbit this reduces to `(f^k(q0), q0]` on the pip's own branch and nothing elsewhere.
- **Words.** A bridge's forward image is read off the dual graph: the REGISTERED prefix is the classes of the consecutive registered crossings on the image unstable branch between `iterate(first, 1)` and `iterate(second, 1)`, sliced BY INDEX out of that branch's ordering (never by a cdist window), each consecutive pair looked up in the class dict; once that runs out, a WALKED suffix crosses filled arc nodes face by face, starting from the `u+` stub of the last registered crossing (`Arrangement.face_at_stub`) or, when neither image is registered, from the arc owning the scaled image cdist. Exactly one distinct word must result, or `DualGraph.walk_bridge` raises `AmbiguousWalkError` listing every candidate; every bridge of one `BridgeClass` must spell the same word, or `SymbolicDynamics` raises `ValueError` naming the class and both words.
- **Endpoint fill and the row invariant.** A walk's endpoint arcs are expected to be hollow (they are images of crossings the trellis has already seen, so they lie at or below the fill boundary), and a filled one is only DEBUG-logged. `row_of_end` reproduces the same-row invariant exactly when the two crossing signs differ — with equal signs the rows come out opposite, which is what a bridge that crosses the stable branch would look like, so the invariant check `check_bridge_rows_consistent` is the geometric guard.
...
| topology | `ElementRef`, `BridgeClass` | `TopologyResults.py`, `BridgeClass.py` | `ElementRef` = the global identity of a partition element, `(branch_key, side, element_id)`; a `BridgeClass` is the ordered pair of `ElementRef`s a bridge connects, resolved by the ROW at each end (see "Bridge classes..." below); `bridge_classes(trellis, partitions)` groups every non-partial bridge id by class |
...
| topology | `DualGraph` | `DualGraph.py` | Bipartite dual of an arrangement: one `FaceNode` per merged face (union-find glues each component's outer face onto a containing face or the global unbounded node), one `ArcNode` per stable `Arc` carrying its element on each side and a `filled` flag; `walk_bridge` spells a bridge's image as a word, raising `AmbiguousWalkError` on more than one distinct word |
| topology | `SymbolicDynamics` | `SymbolicDynamics.py` | The substitution a trellis induces on its bridge classes: `symbols` (label -> `BridgeClass`), `rules` (`"a": "ab"`), `transition_graph`/`transition_matrix`, `describe()`; built by `symbolic_dynamics(dual_graph, classes)` |
| topology | `plotting` | `plotting.py` | All topology drawing routines (Trellis `plot_*` delegate here), including `plot_dual_graph` (hollow/filled arc nodes, face points) and `plot_transition_graph` (the `SymbolicDynamics` digraph) |
| loom | `TangleSession` | `TangleSession.py` | Facade: trellis/arrangement caches keyed by `workbench.generation`, resonance zones, fan-outs over fixed points; `bridge_classes`/`dual_graph`/`symbolic_dynamics` (cached on `(generation, partition signature)`, gathering partitions from every cached per-fixed-point trellis) |
