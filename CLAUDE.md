...
- **Fill.** On stable branch `j`, let `T_j` be its outermost crossing and `T_{j-1}` the outermost crossing of the branch one step back along `advance_key`. The arcs filled are those with cdist above `cdist(f(T_{j-1}))` up to `T_j`. With every branch trimmed at the strong-pip orbit this reduces to `(f^k(q0), q0]` on the pip's own branch and nothing elsewhere.
...
| topology | `plotting` | `plotting.py` | All topology drawing routines (Trellis `plot_*` delegate here), including `plot_dual_graph` (hollow/filled arc nodes, face points) |
| loom | `TangleSession` | `TangleSession.py` | Facade: trellis/arrangement caches keyed by `workbench.generation`, resonance zones, fan-outs over fixed points; `bridge_classes`/`dual_graph` (cached on `(generation, partition signature)`, gathering partitions from every cached per-fixed-point trellis) |
