...
| topology | `plotting` | `plotting.py` | All topology drawing routines (Trellis `plot_*` delegate here), including `plot_dual_graph` (hollow/filled arc nodes, straight face-to-arc edges) and `plot_dual_graph_curved` (each face's unstable boundary arcs drawn as inset copies just inside the face, stubs to the stable arc nodes, so an edge visibly belongs to its face); `dual_graph_legend_handles` |
| loom | `TangleSession` | `TangleSession.py` | Facade: trellis/arrangement caches keyed by `workbench.generation`, resonance zones, fan-outs over fixed points; `bridge_classes` (cached on `(generation, partition signature)`) and `dual_graph` (also keyed on the gathered strong pips), both gathering partitions from every cached per-fixed-point trellis |
