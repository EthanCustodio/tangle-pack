...
- **Fill.** The strong pip `q0` DECLARES which arc nodes are filled (passable to an image bridge at a crossing the trellis has not computed): on the pip's own stable branch, the arcs meeting `(cdist(f^k(q0)), cdist(q0)]` with `k = k_value` (the map steps that return the branch to itself, via `Trellis.image_cdist`), and nothing on any other branch. It is not derived from the outermost crossings, so choosing a different candidate pip moves the fill without trimming; no pip fills nothing and warns. `TangleSession.dual_graph` keys its cache on the gathered pips for that reason.
...
| topology | `DualGraph` | `DualGraph.py` | Bipartite dual of an arrangement: one `FaceNode` per merged face (union-find glues each component's outer face onto a containing face or the global unbounded node), one `ArcNode` per stable `Arc` carrying its element on each side and a `filled` flag; `element_images`, `graph()` (networkx view), `summary()` |
| topology | `plotting` | `plotting.py` | All topology drawing routines (Trellis `plot_*` delegate here), including `plot_dual_graph` (hollow/filled arc nodes, face points, face-to-arc edges) and its `dual_graph_legend_handles` |
| loom | `TangleSession` | `TangleSession.py` | Facade: trellis/arrangement caches keyed by `workbench.generation`, resonance zones, fan-outs over fixed points; `bridge_classes` (cached on `(generation, partition signature)`; letters active classes from the persistent `bridge_alphabet`, records each class's zone) and `dual_graph` (also keyed on the gathered strong pips), both gathering partitions from every cached per-fixed-point trellis; `describe_bridge_classes` |
