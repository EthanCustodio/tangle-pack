...
✅ DONE 2026-09-03 (6.1-6.7). Numbers: k=10 one component, 8 faces = 7 regions + 1 open, 0 regions with an image; nested p3 TWO components (no computed heteroclinic crossing), 30 faces = 27 regions + 1 containing + 2 open, 22 regions with an image, 22/22 agreeing with the mapped representative point. `V - E + F = 2 * components` on both (the traversal gives each component its own outer face). Registry counts unchanged by the anchor work (anchors take the lowest ids and survive `preserve_ids` across a trim; scripts/henon_blast_period_3.py's pinned pip id 10 -> 12, same crossing); inversion bridges 9 -> 7 because `create_bridges` refuses a zero-span cut BETWEEN TWO ANCHORS (closes the degenerate-bridge and `_insert_after` deferred items; any other sub-tolerance pair is still cut, with a WARNING, so a real bridge cannot vanish). Deviations: a face that geometrically encloses another connected component is closed but is NOT a region (the plan's definition) — components come from union-find over the arcs, containment from a ray cast of the other components' nodes, and such faces go to `Arrangement.containing_faces`, leaving `regions` pairwise interior-disjoint; `image_of` accepts the unique closed face carrying the mapped corners in the same cyclic order (strict equality gives 0 images — a trimmed stable manifold hides crossings, so image boundaries carry corners whose preimages were never detected) but then REQUIRES area preservation (rtol 2e-2) plus the forward-mapped representative point to land in it, so a sub-face of a subdivided image is rejected and an image split across several faces returns None (a union result is a dual-graph decision); "every bridge is an arc" is asserted but "every arc is a bridge" only logged (after a blast, crossings born on iterated images have uncut arcs); `representative_point` falls back to a scanline interior point (`polygon_interior_point`, new) when the mean of arc midpoints fails the ray cast (crescent faces); `Region.area`/`contains`/`boundary_points`/`representative_point` raise ValueError on an open face; `Arc.polyline`'s unstable fallback to the parent branch is gated on lo_cdist == 0; the Arrangement is cached on the Trellis snapshot (already per-generation) and exposed as `session.arrangement()`; `Intersection.synthetic` accepts `unstable_manifold`/`unstable_segment` so anchors can be cut at; `registry.find(intersection)` public.
...
✅ DONE 2026-09-03. `grow_until` returns the round count, checks the predicate before growing, and RECUTS existing bridges every round via `_recut_bridges_after_growth` (growing the stable manifold drops new crossings between the endpoints of already-cut bridges, which trips the Arrangement's consecutive-crossings assertion). That recut drops the image bridges and partial pieces from `iterate_bridge`, while `rebuild_bridges` carries the `iterated` flag across by id — so the helper CLEARS the flag on every surviving bridge whose `image_bridges(bid)` is None (otherwise a later blast silently skips it forever) and logs the whole repair at WARNING. `grow_until_iterates_closed("all")` resolves through `registry.from_fixed_point(fixed_point)`, not the whole registry: only the given fixed point's manifolds grow, so a registry-wide set runs to the cap by construction in any multi-saddle session. The set is frozen at call time; unknown ids and an EMPTY resolved set both raise (consistent across both wrappers). `grow_until` forwards `branch_index=None` on an inversion point (one chain covers both branches) and grows only the named branch on a two-branch simple saddle. DEVIATIONS: (a) an anchor's faces never close, but that is a limitation of the current Arrangement anchor model (one anchor per (unstable branch, stable branch) pair, no notion of a branch continuing THROUGH a node, so u-/s- get virtual stubs instead of the other branch's u+/s+ — the "Inversion caveat" deferred item), NOT a consequence of what an anchor is; it is pinned as the cap case. (b) measured on k=10 the open-face corner set is monotone increasing under every growth pattern tried (outer-envelope crossings never become interior, new crossings are born interior), so no growth-driven open->closed transition was found and in practice `grow_until_faces_closed` is a predicate check with a cap: the reachable outcomes are "already interior -> 0 rounds" and the cap. (c) nothing was added to `TangleSession` — the drivers reach it through the existing workbench delegation, and the generation counter already invalidates the cached trellises; only a class-docstring sentence says so.
...
✅ DONE 2026-09-03 (five parallel tasks + a style pass; behaviour-preserving, all 15 scripts output-identical). Landed: one `_collect` walker and `first_node` in BaseManifold; `tanglepack.examples.henon` used by tests/scripts/viz (`Stability` defined once in numerics/Intersection.py); TangleWorkbench 2469 -> ~1575 lines with `BridgeIterator`, `IterateInference`, `graphviz` split out; `topology/plotting.py` holds every plotter (Trellis/StablePartition delegate), `TangleSession._fanout_plot`/`_fanout_call`; Trellis `verbose` prints are INFO logs; every module starts with a docstring + `from __future__ import annotations`, Dev Notes above imports, inline TODOs converted, `Tangle` NullHandler, stale WARNING block gone, refinement notes reconciled (ManifoldMachine's "not a bug, the arm escapes" diagnosis kept), commented-out code removed from src and 8 scripts, ruff -F clean; four dead notebooks archived under notebooks/archive, three scripts/*.ipynb migrated and re-executed, seven partial plans annotated with a Status block, Sphinx stubs regenerated (nbconvert/sphinx installed into env/). Deviations: `_require_manifold` is a private workbench method (all call sites are there), no `_guards.py`; `plot_stable_partition` keeps its results-taking signature; `verbose` kwargs kept (scripts use them); `Trellis.scale_cdist` and `FixedPoint.get_iterable_array(shift=)` were NOT deleted — both have test callers, the Deferred entry below was wrong; `numerics/__init__` exports `BridgeIterator`/`IterateInference` for `__all__` symmetry.
...
- Phase 6: on an inversion point the arrangement has four coincident anchor nodes (one per (u-branch, s-branch) pair) instead of one degree-4 node whose u-/s- are branch 1's u+/s+; needs a "branch continues through the node" notion on Intersection/TrellisBranch.
- Phase 6: `notebooks/intersection_sandbox.ipynb` imports shapely (Phase 8 notebook pass); the blast still logs 5 `'NoneType' object has no attribute 'cdist'` bridge-iterate failures — unexamined, pre-existing (count varies run to run, as p3 registry ids are not deterministic).
- Phase 6: an image subdivided across SEVERAL faces returns None from `image_of` rather than a union of faces; deciding what a union-valued image means belongs to the dual graph.
- Phase 6: `Trellis.scale_cdist` still has no production caller, and `registry.by_*_cdist`/`*_rank` are still global-order views — the region layer takes all per-branch order from `TrellisBranch.ordered_ids`.
- Phase 6: `test_registry_insert_is_near_linear` was a pre-existing 1-in-3 flake in full-suite runs (heap fragmentation); `gc.collect()` before timing added.
- Phase 8: `ax` parameters untyped in topology/plotting.py, Trellis.py, TangleSession.py; `tests/numerics/test_segment_index_bookkeeping.py` docstring still names the deleted `_segments_touching`; `scripts/henon_initial_3_points.py` imports pyplot and `tangle_workbench_test.py` imports networkx unused; notebooks/henon_nested_period_3.ipynb and scripts/*.ipynb still define their own Hénon (should import tanglepack.examples); `saddle_guesses` lacks the k=2.8 / k=2.1 entries; add `nbconvert`/`sphinx`/`furo`/`myst_parser` as a `docs` optional-dependency; `grow_until_*` default `max_iterations=10` is too low for period>1 (notebooks pass 20).
- Phase 8: `notebooks/henon_nested_period_3.ipynb` (the user's working notebook, left untouched by this branch beyond the user's own uncommitted hunks) has stale cell-0 debug probes (`_strictify_increasing` hasattr, `inspect.getsource(_refine_layer)`), `USE_GPU` hard-forced to "1", and a "k=2.1" comment next to `k = 2.8`; fix when the user is ready to re-execute it.
- Phase 8: `Trellis` `verbose=True` prints only when logging is unconfigured (one deliberate `print` in Trellis._log_report); with a configured logger the report is an INFO record.
